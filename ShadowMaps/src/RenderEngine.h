...
public:
	...
	void display(WorldState & state)
	{
        ...
        glUniformMatrix4fv(glGetUniformLocation(shaderProg[sid],"trans"),1,GL_FALSE,&trans[0][0]);
...
//		glDrawElements(GL_TRIANGLES, state.getModel().getElements().size(), GL_UNSIGNED_INT, 0);
         ...
         trans = glm::translate(glm::mat4(1), glm::vec3(10,0,0));
        glUniformMatrix4fv(glGetUniformLocation(shaderProg[sid],"trans"),1,GL_FALSE,&trans[0][0]);
...
private:
    ...
    void setupBuffers2(Model & model2){
        ...
        glBufferData(GL_ARRAY_BUFFER, model2.getPositionBytes(), &model2.getPosition()[0], GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER, model2.getColorBytes(), &model2.getColor()[0], GL_STATIC_DRAW);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, model2.getElementBytes(), &model2.getElements()[0], GL_STATIC_DRAW);
...
//        glVertexAttribPointer(positionSlot2, 3, GL_FLOAT, GL_FALSE, 0, 0);
	...
	void uploadUniforms(GLuint shaderId, WorldState const & state)
	{
        ...
        glUniform1i(glGetUniformLocation(shaderId,"shadingMode"),state.getShadingMode());
